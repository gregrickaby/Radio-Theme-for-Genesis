...
/**
 * Radio Latest Posts widget class.
 *
 * @package Radio
 * @subpackage Widgets
 *
 * @since 1.0.0
 */
class Radio_Latest_News extends WP_Widget {
	...
	function __construct() {
		...
		$widget_ops = array(
			'classname'   => 'radio-latest-news',
			'description' => __( 'Displays the latest news with thumbnails', 'genesis' ),
		);

		$control_ops = array(
			'id_base' => 'radio-latest-news',
			'width'   => 505,
			'height'  => 350,
		);

		$this->WP_Widget( 'radio-latest-news', __( 'Radio - Latest News', 'genesis' ), $widget_ops, $control_ops );
	...
	function widget( $args, $instance ) {
		...
		if ( $latest_news->have_posts() ) : while ( $latest_news->have_posts() ) : $latest_news->the_post();
			...
			if ( $loop_counter == $instance['posts_num'] ) { $classes .= ' last'; }
